...
if ( ! function_exists( 'us_list_filter_get_post_count' ) ) {
	...
	function us_list_filter_get_post_count( $query_args, $list_filters = array() ) {
		...
		$filter_names = $filters_with_minmax = array();

		foreach ( $list_filters as $filter_name => $filter_type ) {
			$name = strtok( $filter_name, '|' );

			if ( $filter_type == 'range_slider' ) {
				$filters_with_minmax[] = $name;
			}
		...
		foreach ( $all_filters as $filter_name => $filter_values ) {
			foreach ( $filter_values as $filter_value => $post_ids ) {
				...
				// Filters with minmax don't need post count, but min and max values
				if ( in_array( $filter_name, $filters_with_minmax ) ) {
					if ( $post_count ) {
		...
		// Collect min and max values for specific filters
		foreach ( $filters_with_minmax as $filter_name ) {
