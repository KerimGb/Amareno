...
! function( $, _undefined ) {
	...
	function usListFilter( container ) {
		...
		$( '.type_range_slider', container ).each( ( _, filter ) => {
			function showFormattedResult( _, ui ) {
				$( '.for_min_value, .for_max_value', filter ).each( ( i, node ) => {
					var value = $ush.toString( ui.values[ i ] );
		...
		if ( self.isFacetedFiltering ) {
			...
			const listData = self.firstListData();
			...
			self.firstList().trigger( 'usListFilter', { list_filters: listFilters } );
	...
	$.extend( usListFilter.prototype, $ush.mixinEvents, {
		...
		firstListData: function() {
			return $ush.toPlainObject( ( this.firstList().data( 'usPostList' ) || {} ).data );
		},
		...
		setPostCount: function( data ) {
			...
			$.each( self.$filters, ( filterName, filter ) => {
				...
				if ( $filter.hasClass( 'type_checkbox' ) || isRadioButtons ) {
					...
					var newValues;
					if ( currentValues ) {
						newValues = parseValues( currentValues );
					} else {
		...
		applyFilterToList: function() {
			...
			self.firstList().trigger( 'usListFilter', self.result );
		},
