...
var strValues=$ush.toString((value.match(/^linear-gradient\((.*)\)$/)||[])[1]),iteration=0;while(strValues&&iteration++<15){var part=$ush.toString((strValues.match(/^(rgba?\([\d\,\.]+\)(\s[^\,]+)?|([^\,]+)),?/)||[])[0]).trim(REGEXP_SPACE);strValues=strValues.replace(part,'').trim(REGEXP_SPACE);if(part.slice(-1)===','){part=part.slice(0,-1)}
...
const colorValue=Helper.isVariable(itemValue)?Helper.valueToPreview(itemValue):itemValue;if(!result.firstValue){result.firstValue=itemValue;result.firstColorValue=colorValue}
...
mediaInstance.state().get('selection').add(wp.media.attachment($ush.parseInt(attachmentId)))})},_selectedInMediaLibrary:function(){var self=this,mediaInstance=self.mediaInstance,value=[];if(self.isMultiple){mediaInstance.state().get('selection').each(function(attachment){value.push(attachment.id)})}else{var attachment=mediaInstance.state().get('selection').first();if(attachment.attributes.url){value=[attachment.id]}}
if(_window.vc_selectedFilters&&_window.vcAdminNonce){$.ajax({type:'POST',url:$usof.ajaxUrl,dataType:'json',data:{action:'vc_media_editor_add_image',filters:_window.vc_selectedFilters,ids:value,_vc_inline:!0,_vcnonce:_window.vcAdminNonce},success:(res)=>{if(res.data.ids.length){self.setValue(res.data.ids.toString())}},complete:()=>{_window.vc_selectedFilters=_undefined},})}else{self.setValue(value.toString())}},_removeItem:function(e){var self=this;$previewFile=$(e.target).closest('.usof-upload-preview-file'),removeValue=$ush.toString($previewFile.data('value'));if(removeValue){var value=$ush.parseInt(removeValue)!==-1?$ush.toString(self.getValue()):'';if(value){value=value.split(',').filter(function(value){return value!==removeValue}).toString()}
...
self.$draggedElm.removeAttr('style').insertBefore(self.$dragShadow);self.$dragShadow.detach();var values=[];$('.usof-upload-preview-file',self.$preview).each(function(_,node){var value=$ush.toString(node.dataset.value);if(value){values.push(value)}});if(values.length){self.parentSetValue(values.toString())}else{self.setValue('')}}})}(jQuery);!function($,_undefined){var _window=window,_document=document;_window.$ush=_window.$ush||{};if($ush.isUndefined(_window.$usof)){return}
