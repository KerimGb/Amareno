...
self.removeDragScrollData();$usb.postMessage('doAction','_scrollDragging')}});$.extend(prototype,{isProcessSave:function(){return _$tmp.isProcessSave},savePageData:function(complete){var self=this;var data={pageMeta:{},};if(self.isContentChanged()){data.post_content=self.pageData.content}
...
if(!(_$tmp.pageFieldset.pageMeta instanceof $usof.GroupParams)){var pageMeta=new $usof.GroupParams($('.usb-panel-page-meta',self.$pageSettings)[0]);for(var k in pageMeta.fields){pageMeta.fields[k].on('change',$ush.debounce(self._events.changePageMeta,1))}
...
var pageData=$usb.builder.pageData,pageMeta=_$tmp.pageFieldset.pageMeta,pageFields=_$tmp.pageFieldset.pageFields;if(pageFields instanceof $usof.GroupParams){pageFields.setValues(pageData.fields,!0);$.extend(pageData.fields,pageFields.getValues())}
...
$ush.debounce_fn_1ms(()=>{$usb.trigger('builder.contentChange')})},_changePageMeta:function(usofField,value){if(!(usofField instanceof $usof.field)){return}
