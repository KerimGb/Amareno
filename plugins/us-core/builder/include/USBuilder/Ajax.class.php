...
final class USBuilder_Ajax {
	...
	static function save_post() {
		...
		$metadata = us_arr_path( $_POST, 'pageMeta', array() );

		// Update post metadata
		if ( ! empty( $metadata ) AND is_array( $metadata ) ) {
			...
			foreach ( us_config( 'meta-boxes', array() ) as $meta_box ) {
				...
				foreach ( $meta_box['fields'] as $key => $field ) {
					$default_metaboxes[ $key ] = us_arr_path( $field, 'std' );
				}
			}


			$post->meta_input = $post->meta_input ?? array();
			foreach ( $metadata as $meta_key => $meta_value ) {
				if ( ! isset( $default_metaboxes[ $meta_key ] ) ) {
					continue;
				}

				// Save the meta field only with non-default value
				if ( ! is_null( $meta_value ) AND $meta_value != $default_metaboxes[ $meta_key ] ) {
